...
class MeetingUI:
            ...
            def on_recording_saved():
                ...
                if self.current_session_id:
                    ...
                    if session and self.meeting_recorder.session_info:
                        session.audio_file_path = self.meeting_recorder.session_info.audio_file_path
                        self.session_manager.save_session_state()
                    # Ensure processor has the active session id
                    try:
                        self.meeting_processor.current_session_id = self.current_session_id
                    except Exception:
                        pass
                    # Auto-load the recorded audio into the waveform editor on the UI thread
                    try:
                        if self.waveform_editor and self.root and self.root.winfo_exists() and session and session.audio_file_path:
                            self.root.after(0, lambda: self.waveform_editor.load_audio_file(session.audio_file_path))
                    except (tk.TclError, RuntimeError):
                        pass
                
                # Process meeting in background after file is saved if configured
                if meeting_config.AUTO_CHUNK_ON_STOP:
    ...
    def _on_processing_complete(self):
        ...
        self.save_meeting_button.config(state=tk.DISABLED)
        self._update_session_info_display()
        # Ensure waveform editor shows the processed audio and chunk markers
        try:
            if self.current_session_id and self.waveform_editor:
                session = self.session_manager.get_session(self.current_session_id)
                if session and session.audio_file_path:
                    self.waveform_editor.load_audio_file(session.audio_file_path)
        except Exception as e:
            logging.warning(f"Failed to refresh waveform editor after processing: {e}")
    ...
    def load_audio_in_editor(self):
        ...
        try:
            session = self.session_manager.get_session(self.current_session_id)
            if session and session.audio_file_path:
                # Ensure processor has the active session id for editor chunk ops
                try:
                    self.meeting_processor.current_session_id = self.current_session_id
                except Exception:
                    pass
    ...
    def export_audio_chunks(self):
        ...
        try:
            session = self.session_manager.get_session(self.current_session_id)
            if not session:
                messagebox.showwarning("No Session", "Session not found.")
                return
            if not session.chunks_directory or not os.path.isdir(session.chunks_directory):
                messagebox.showinfo("No Chunks", "No chunks available to export. Auto-chunk first or apply manual changes.")
                return
            # Open chunks folder
            import platform, subprocess
            if platform.system() == "Windows":
                os.startfile(session.chunks_directory)
            elif platform.system() == "Darwin":
                subprocess.call(["open", session.chunks_directory])
            else:
                subprocess.call(["xdg-open", session.chunks_directory])
        except Exception as e:
            logging.error(f"Failed to open chunks directory: {e}")
            messagebox.showerror("Error", f"Failed to open chunks directory: {e}")
    ...
    def transcribe_all_chunks(self):
        ...
        if not self.current_session_id:
            ...
            return
        try:
            self.status_controller.update_status("Starting bulk transcription...")
            self.executor.submit(self._transcribe_all_chunks_background)
        except Exception as e:
            logging.error(f"Failed to start bulk transcription: {e}")
            messagebox.showerror("Error", f"Failed to start transcription: {e}")

    def _transcribe_all_chunks_background(self):
        """Background worker to transcribe all chunks and save transcript."""
        try:
            if not self.current_session_id:
                return
            session = self.session_manager.get_session(self.current_session_id)
            ...
            chunks = []
            try:
                chunks = self.meeting_processor.get_chunks(self.current_session_id)
            except Exception:
                chunks = []

            if not chunks:
                safe_status("Preparing chunks for transcription...")
                self.meeting_processor.process_meeting_audio(
                    session.audio_file_path,
                    self.current_session_id,
                    lambda m: safe_status(m)
                )
                try:
                    chunks = self.meeting_processor.get_chunks(self.current_session_id)
                except Exception:
            ...
            if not chunks:
                safe_status("No chunks available for transcription")
                return

            chunk_files = [c.file_path for c in chunks if c.file_path and os.path.exists(c.file_path)]
            if not chunk_files:
                safe_status("Chunk files missing; cannot transcribe")
                return

            # Transcribe using local Whisper backend
            safe_status("Transcribing chunks with Whisper...")
            from transcriber.local_backend import LocalWhisperBackend
            backend = LocalWhisperBackend()
            if not backend.is_available():
                safe_status("Local Whisper unavailable (ffmpeg/model). Check settings.")
                return
            combined_text = backend.transcribe_chunks(chunk_files)

            # Save transcript alongside the audio file
            audio_dir = os.path.dirname(session.audio_file_path)
            base_name = os.path.splitext(os.path.basename(session.audio_file_path))[0]
            transcript_path = os.path.join(audio_dir, f"{base_name}.txt")
            try:
                with open(transcript_path, 'w', encoding='utf-8') as f:
                    f.write(combined_text)
                session.transcript_file_path = transcript_path
                self.session_manager.save_session_state()
            except Exception as e:
                logging.error(f"Failed to save transcript: {e}")

            safe_status("Transcription complete")
        except Exception as e:
            logging.error(f"Bulk transcription failed: {e}")
            try:
                if self.root and self.root.winfo_exists():
