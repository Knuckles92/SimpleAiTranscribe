...
class MeetingProcessor(AudioProcessor):
    ...
    def get_chunks(self, session_id: Optional[str] = None) -> List[ChunkMetadata]:
        """Return current chunk metadata for a session (sorted by start time).
        
        Args:
            session_id: Optional session id. Uses current session if omitted.
        
        Returns:
            List of ChunkMetadata entries.
        """
        if session_id is None:
        ...
        # Prefer in-memory chunks
        session_chunks = self.session_chunks.get(session_id)
        if session_chunks:
            return sorted(list(session_chunks.values()), key=lambda c: c.start_timestamp)

        # Attempt to reconstruct from files if available
        session = self.session_manager.get_session(session_id)
        if not session:
            return []
        ...
        try:
            audio_data, sample_rate = self._load_audio_data(session.audio_file_path)
            total_duration = len(audio_data) / sample_rate
        except Exception:
            total_duration = 0.0

        chunks: List[ChunkMetadata] = []
        # If we have stored chunk files, approximate sequential boundaries
        if session.chunk_files:
            current_start = 0.0
            for idx, file_path in enumerate(session.chunk_files):
                try:
                    chunk_audio, sr = self._load_audio_data(file_path)
                    duration = len(chunk_audio) / sr
                    start_ts = current_start
                    end_ts = min(start_ts + duration, total_duration) if total_duration > 0 else start_ts + duration
                    chunk = ChunkMetadata(
            ...
            # Cache reconstructed chunks in memory for subsequent calls
            if chunks:
                self.session_chunks[session_id] = {c.chunk_id: c for c in chunks}
                return chunks

        return []

    def update_chunks(self, chunks: List[ChunkMetadata], session_id: Optional[str] = None) -> bool:
        """Persist new chunk boundaries by recreating chunk audio files.
        
        This converts the provided boundary list into physical chunk files under
        the session's chunks directory and updates in-memory metadata.
        
        Args:
            chunks: Desired chunk boundaries (start/end in seconds).
            session_id: Optional session id. Uses current session if omitted.
        
        Returns:
            True if chunk update succeeded.
        """
        if session_id is None:
            session_id = self.current_session_id
        if not session_id:
            logging.warning("update_chunks called without an active session")
            return False

        session = self.session_manager.get_session(session_id)
        if not session:
            logging.warning(f"Session {session_id} not found in update_chunks")
            return False
        if not session.audio_file_path or not os.path.exists(session.audio_file_path):
            logging.warning("Session audio file missing; cannot update chunks")
            return False

        try:
            # Load full meeting audio once
            audio_data, sample_rate = self._load_audio_data(session.audio_file_path)
            total_samples = len(audio_data)

            # Ensure chunks directory
            if not session.chunks_directory:
                session.chunks_directory = os.path.join("meetings", "chunks", session_id)
            os.makedirs(session.chunks_directory, exist_ok=True)

            # Remove previous chunk files (best-effort)
            try:
                for existing in list(self.session_chunks.get(session_id, {}).values()):
                    try:
            ...
            # Sort and clamp boundaries
            safe_chunks: List[ChunkMetadata] = []
            for c in sorted(chunks, key=lambda x: x.start_timestamp):
                start_s = max(0.0, float(c.start_timestamp))
                end_s = max(start_s, float(c.end_timestamp))
                if end_s - start_s <= 0.0:
                    continue
                safe_chunks.append(ChunkMetadata(
            ...
            for idx, c in enumerate(safe_chunks):
                start_sample = int(c.start_timestamp * sample_rate)
                end_sample = int(c.end_timestamp * sample_rate)
                start_sample = max(0, min(start_sample, total_samples))
                end_sample = max(start_sample + 1, min(end_sample, total_samples))

                chunk_audio = audio_data[start_sample:end_sample]
                out_path = os.path.join(session.chunks_directory, f"chunk_{idx:03d}.wav")
                self._save_audio_chunk(chunk_audio, sample_rate, out_path)

                c.file_path = out_path
                c.duration = (end_sample - start_sample) / sample_rate
                c.updated_at = datetime.now().isoformat()
                new_metadata.append(c)

            # Update in-memory map and session
            self.session_chunks[session_id] = {c.chunk_id: c for c in new_metadata}
            session.chunk_files = [c.file_path for c in new_metadata]
            self.session_manager.save_session_state()

            logging.info(f"Updated chunks for session {session_id}: {len(new_metadata)} chunks")
            return True
        except Exception as e:
            logging.error(f"Failed to update chunks for session {session_id}: {e}")
            return False
    ...
    def _find_meeting_split_points(self, audio_data: np.ndarray, sample_rate: int,
        ...
        # Faster moving average (500ms) using cumulative sum instead of convolution
        window_size = max(1, int(0.5 * sample_rate))
        if window_size > 1:
            csum = np.cumsum(np.insert(audio_abs, 0, 0.0))
            ma = (csum[window_size:] - csum[:-window_size]) / window_size
            # Pad to original length (centered approx.)
            pad_left = window_size // 2
            pad_right = len(audio_abs) - (len(ma) + pad_left)
            audio_smooth = np.pad(ma, (pad_left, max(0, pad_right)), mode='edge')
        else:
